...
class GemmaTest(unittest.TestCase):
  ...
  def test_gemma(self):
    ...
    states, ms_func = torch4ms.extract_mindspore(model, env)
    inputs_ms = env.t2ms_copy(inputs)
    result_ms = ms_func(states, inputs_ms, None)
    # result_ms is mindspore tensor(s); convert to numpy for inspection
    if hasattr(result_ms, 'asnumpy'):
      print(result_ms.asnumpy())
    else:
      print(result_ms)
