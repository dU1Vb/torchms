...
# Try to import JAX, but handle case when it's not available
JAX_AVAILABLE = False
try:
  import jax
...
class Tensor(torch.Tensor):
  ...
  def __init__(self, elem: ms.Tensor, env: "Environment", requires_grad=False):
    super().__init__()
    self._elem = elem
    self._env = env

  def __str__(self):
    if isinstance(self._elem, torch.Tensor):
      return f"Torch Tensor of {type(self._elem)}: {self._elem.dtype}{self._elem.shape}"
    elif isinstance(self._elem, ms.Tensor):
      return f"MindSpore Tensor of {type(self._elem)}: {self._elem.dtype}{self._elem.shape}"
    elif JAX_AVAILABLE and isinstance(self._elem, jax.Array):
      return f"Jax Array of {type(self._elem)}: {self._elem.dtype}{self._elem.shape}"
    else:
      return f"Unknown Tensor type: {type(self._elem)}"
  ...
  def __setitem__(self, key, val):
    if JAX_AVAILABLE:
      # Use JAX array indexing if available
      key, val = self._env.t2j_iso((key, val))
      self._elem = self._elem.at[key].set(val)
    else:
      # Fallback to MindSpore indexing when JAX is not available
      key, val = self._env.t2ms_iso((key, val))
      self._elem[key] = val
  ...
  def detach(self):
    return Tensor(mops.stop_gradient(self.mindspore()), self._env)

  def numpy(self) -> numpy.ndarray:
    import numpy as np

    return np.array(self._elem.asnumpy())

  def mindspore(self) -> ms.Tensor:
    return self._elem
  ...
  def jax(self):
    ...
    Returns:
      jax.Array: The tensor converted to JAX format.
    
    Raises:
      ImportError: If JAX is not available.
    """
    if not JAX_AVAILABLE:
      raise ImportError("JAX is not available. This method requires JAX.")
    return mappings.t2j(self._elem)
    
  def torch(self) -> torch.Tensor:
    if JAX_AVAILABLE:
      # Use JAX-based conversion if available
      return self._env.j2t_copy(self.jax())
    else:
      # Fallback to direct MindSpore to torch conversion
      return self._env.ms2t_copy(self._elem)
  ...
  @property
  def device(self):
    return torch.device("mindspore:0")

  @property
  def mindspore_device(self):
  ...
  def apply_mindspore(self, mindspore_function, *args, **kwargs):
    # Call a mindspore function on _elem
    res = mindspore_function(self._elem, *args, **kwargs)
    return self._env.ms2t_iso(res)

  def apply_mindspore_(self, mindspore_function, *args, **kwargs):
    self._elem = mindspore_function(self._elem, *args, **kwargs)
    return self

  def apply_jax(self, jax_function, *args, **kwargs):
    """Apply a jax function to the underlying tensor.
    
    Args:
      jax_function: The JAX function to apply.
    ...
    if not JAX_AVAILABLE:
      raise ImportError("JAX is not available. This method requires JAX.")
    # Make sure the element is a JAX array before applying the function
    if isinstance(self._elem, ms.Tensor):
      jax_elem = mappings.t2j(self._elem)
    else:
      jax_elem = self._elem
    res = jax_function(jax_elem, *args, **kwargs)
    return Tensor(res, self._env)
    
  def apply_jax_(self, jax_function, *args, **kwargs):
    """Apply a jax function to the underlying tensor in-place.
    ...
    Returns:
      Tensor: The modified tensor (self).
  ...
  def shard_(self, sharding):
    """Shard the tensor according to the given sharding spec.

    Args:
      sharding: A PartitionSpec object or similar sharding specification.
    ...
    if not JAX_AVAILABLE:
      raise ImportError("JAX is not available. This method requires JAX.")
    # MindSpore doesn't have direct equivalent of jax.lax.with_sharding_constraint
    # This would need to be implemented with MindSpore's parallel mechanisms
    return self
...
def _make_debug_msg(is_dispatch, log_args, func, args, kwargs):
  ...
  def _display(a):
    ...
    elif JAX_AVAILABLE and isinstance(a, jax.Array):
...
class XLAFunctionMode(torch.overrides.TorchFunctionMode):
  ...
  def __torch_function__(self,
    ...
    if not JAX_AVAILABLE:
      # Fallback to torch implementation if JAX is not available
      return torch.overrides.call_torch_function(func, types, args, kwargs)
...
class XLADispatchMode(torch_dispatch.TorchDispatchMode):
  ...
  def __torch_dispatch__(self, func, types, args=(), kwargs=None):
    if not JAX_AVAILABLE:
      # Fallback to default dispatch if JAX is not available
      return torch_dispatch.default_torch_dispatch(func, types, args, kwargs)
...
SUPPORTED_MINDSRORE_PLATFORM = ["cpu", "gpu", "ascend"]
...
class RuntimeProperty:
  ...
  def get_and_rotate_prng_key(self):
    """Get the current PRNG key and rotate it to a new one.
...
class Environment(contextlib.ContextDecorator):
  ...
  def __init__(self, configuration=None):
    ...
    # MindSpore uses a different random number generation system
    # For simplicity, we'll just store the seed
    _prng_seed = torch.initial_seed() % (1 << 63)
    self._property = threading.local()
    self._property.content = [
        RuntimeProperty(
            mesh=_mesh, prng=_prng_seed, autocast_dtype=autocast_dtype)
    ]
  ...
  def manual_seed(self, key):
    if isinstance(key, torch.Tensor):
        ...
        if isinstance(key, Tensor):
            key = key._elem.asnumpy().item()
        else:
            key = key.item()
    # In MindSpore, we just use the seed directly
  ...
  def _is_same_device(self, the_tensor, new_device):
    if new_device is None:
      return True
    if new_device == 'meta' and the_tensor.device.type == 'mindspore':
      return True
  ...
  def _to_copy(self, the_tensor, new_dtype, new_device):
    ...
    if not self._is_same_device(the_tensor, new_device):
      if isinstance(the_tensor, Tensor):
        torch_tensor = self.ms2t_copy(the_tensor._elem)
        with mode_utils.no_dispatch(), torch._C.DisableTorchFunction():
          return torch_tensor.to(device=new_device, dtype=new_dtype)
      else:
        arr = self.t2ms_copy(the_tensor)
    ...
    if new_dtype is not None and new_dtype != res.dtype:
      if isinstance(res, Tensor):
        if JAX_AVAILABLE:
          ...
          # Fallback to MindSpore implementation when JAX is not available
          # Get the corresponding MindSpore dtype
          ms_dtype = mappings.t2ms_dtype(new_dtype)
          res = Tensor(res._elem.astype(ms_dtype), self, res.requires_grad)
      else:
        with mode_utils.no_dispatch(), torch._C.DisableTorchFunction():
  ...
  def get_and_rotate_prng_key(self,
    ...
    if generator is not None:
      return generator.initial_seed() % (2**63)
    # For MindSpore, we'll just return the seed and increment it
    # This is a simplified implementation
    current_seed = self.param.prng
    # Increment seed for next use
    new_prop = self.param.override(prng=current_seed + 1)
    self._property.content[-1] = new_prop
    return current_seed
    ...
    if self._should_use_torchax_tensor(device):
      ...
      if op.is_jax_function:
        if JAX_AVAILABLE:
          ...
          # Skip JAX tensor conversion if JAX is not available
          pass
      res = op.func(*args, **kwargs)
      if JAX_AVAILABLE and isinstance(res, jax.Array):
  ...
  def dispatch(self, func, types, args, kwargs):
    ...
    # MindSpore doesn't have direct equivalent of jax.named_scope
    # We'll just proceed without it
      ...
      try:
        ...
        if op.is_jax_function and JAX_AVAILABLE:
  ...
  def enable_torch_modes(self):
    # Only set modes if JAX is available
    if JAX_AVAILABLE:
  ...
  def disable_torch_modes(self, *exc):
    if not exc:
      exc = (None, None, None)
    # Only exit modes if JAX is available
    if JAX_AVAILABLE:
  ...
  def _move_one_value(self, val):
    ...
    if isinstance(val, torch.Tensor):
      return Tensor(self.t2ms_copy(val), self)
    return val
  ...
  def t2ms_iso(self, torchtensors):
    """Convert torchax Tensor to mindspore tensor.

    This function will not copy, will just unwrap the inner mindspore tensor out.
    ...
    def to_mindspore(x):
      ...
      assert isinstance(x, Tensor) or isinstance(x, View), (
          f"Expect a Tensor or a View but got {type(x)}; usually this means there is a mixed math between MindSporeTensor and torch.Tensor"
      )
      return x.mindspore()

    res = torch_pytree.tree_map_only(torch.Tensor, to_mindspore, torchtensors)
    return res
  ...
  def ms2t_iso(self, mindspore_tensor):
    """Convert mindspore tensor to torchax Tensor.

    This function will not copy, will just wrap the mindspore tensor with a torchax Tensor
    Note: iso is short for "isomorphic"
    """
    return torch_pytree.tree_map_only(ms.Tensor, lambda x: Tensor(x, self),
                                      mindspore_tensor)

  def ms2t_copy(self, args):
    """Convert mindspore tensor to torch.Tensor in cpu
  ...
  def t2j_iso(self, torchtensors):
    ...
    def to_jax(x):
      ...
      if isinstance(
        ...
        x = x.wait()
      assert isinstance(x, Tensor) or isinstance(x, View), (
          f"Expect a Tensor or a View but got {type(x)}; usually this means there is a mixed math between JAX Array and torch.Tensor"
      )
      return x.jax()
